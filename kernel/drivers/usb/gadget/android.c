...
void samsung_enable_function(int mode)
{
	...
	switch(mode) {
		case USBSTATUS_UMS:
			...
			printk("*********** LNT DEBUG *********** : UMS : %s \n",__func__);
			ret = set_product(dev, USBSTATUS_UMS);
			break;
#ifdef _SUPPORT_SAMSUNG_AUTOINSTALLER_
		case USBSTATUS_SAMSUNG_KIES:
			CSY_DBG_ESS("mode = USBSTATUS_SAMSUNG_KIES (0x%x)\n", mode);
			printk("*********** LNT DEBUG *********** : KIES  : %s \n",__func__);
			ret = set_product(dev, USBSTATUS_SAMSUNG_KIES);
			break;
		case USBSTATUS_SAMSUNG_KIES_REAL:
			CSY_DBG_ESS("mode = USBSTATUS_SAMSUNG_KIES REAL (0x%x)\n", mode);
			printk("*********** LNT DEBUG *********** : KIES REAL : %s \n",__func__);
			ret = set_product(dev, USBSTATUS_SAMSUNG_KIES);
			break;
#else
		case USBSTATUS_SAMSUNG_KIES:
		...
		case USBSTATUS_MTPONLY:
			...
			printk("*********** LNT DEBUG *********** : MTP : %s \n",__func__);
			ret = set_product(dev, USBSTATUS_MTPONLY);
			break;
		case USBSTATUS_ADB:
			CSY_DBG_ESS("mode = USBSTATUS_ADB (0x%x)\n", mode);
			printk("*********** LNT DEBUG *********** : ADB : %s \n",__func__);
			ret = set_product(dev, USBSTATUS_ADB);
			break;
#if !defined(CONFIG_ARIES_NTT) // disable tethering
		...
		case USBSTATUS_ASKON: /* do not save usb mode */
			printk("********** LNT DEBUG ******* USBSTATUS ASKON : %s \n",__func__);
...
static ssize_t UsbMenuSel_switch_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	printk("******************* UsbMenuSel_switch_store =***************** (%s)\n", buf);

#ifdef _SUPPORT_SAMSUNG_AUTOINSTALLER_
	if (strncmp(buf, "KIES_REAL", 9) == 0){
                CSY_DBG_ESS("Enable KIES REAL(%d)\n", value);
                printk("*********** LNT DEBUG : KIES REAL : %s \n",__func__);
		askon_status=0;
                //UsbMenuSelStore(0);
                samsung_enable_function(USBSTATUS_SAMSUNG_KIES_REAL);
        }
#endif
