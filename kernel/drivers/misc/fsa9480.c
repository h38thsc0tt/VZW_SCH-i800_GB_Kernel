...
/* uevent for Tool launcher for VZW */
#ifdef CONFIG_TARGET_LOCALE_VZW
...
unsigned int FSA9480_Get_JIG_Status(void)
{

        struct i2c_client *client = local_usbsw->client;
        unsigned int device2;

        device2 = i2c_smbus_read_byte_data(client,FSA9480_REG_DEV_T2);
        if((device2 & (DEV_JIG_USB_ON | DEV_JIG_USB_OFF)) && HWREV >= 13)
                return 1;
        else
                return 0;
}
...
/* uevent sent only in case of VZW for USB cable connected for Tool launcher */
#ifdef CONFIG_TARGET_LOCALE_VZW
			printk("$$$$$$$$$$$$$$$$ Sending connection event $$$$$$$$$$$$$$ \n");
			switch_set_state(&sdev,1); // sending event to platform on attaching USB
#endif
#ifdef _SUPPORT_SAMSUNG_AUTOINSTALLER_
...
/* uevent sent only in case of VZW for USB cable detached for Tool launcher */
#ifdef CONFIG_TARGET_LOCALE_VZW
			printk("$$$$$$$$$$$$$$$$ Sending detached  event $$$$$$$$$$$$$$ \n");
			switch_set_state(&sdev,0); // sending event to platform on disconnecting USB
#endif
		/* UART */
...
/* usb_attach event used only for Tool Launcher in VZW*/
#ifdef CONFIG_TARGET_LOCALE_VZW  
	sdev.name = "usb_attach";
