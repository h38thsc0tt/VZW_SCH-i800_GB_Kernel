...
unsigned int batt_slate_mode; /* slate mode */
#endif
...
#endif
#ifdef FEATURE_SPRINT_SLATE
        SEC_BATTERY_ATTR(batt_slate_mode), /* slate mode */
#endif
};
...
#ifdef FEATURE_SPRINT_SLATE
		if(chg->bat_info.batt_slate_mode)
			val->intval = 0;		
#endif
		break;
				   ...
				   const char *buf, size_t count)
{
	...
	case CHARGING_MODE_BOOTING:
        ...
        case BATT_SLATE_MODE:
                        if (sscanf(buf, "%d\n", &x) == 1) {
                                batt_slate_mode = x;
				chg->bat_info.batt_slate_mode = x;
                                ret = count;
                        }
                                printk("%s: batt_slate_mode = %d\n", __func__, x);
                        break;
	#endif
...
#ifdef FEATURE_SPRINT_SLATE
        chg->bat_info.batt_slate_mode = 0;
#endif
